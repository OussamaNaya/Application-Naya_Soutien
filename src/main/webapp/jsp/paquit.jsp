...
<body>
    ...
    <nav class="navbar navbar-expand-lg bg-white navbar-light shadow sticky-top p-0">
        ...
        <% 
        	...
        	{   
        		 if (request.getParameter("op") != null && request.getParameter("op").equals("supprimer")) {
        		        int idc = Integer.parseInt(request.getParameter("id"));
        		        Cour c = daoc.Find_ID(idc);
        		        
        		        List<Cour> lc2 = new ArrayList<Cour>();
        		        if (session.getAttribute("lc") != null) {
        		            lc2 = (List<Cour>) session.getAttribute("lc");
        		            lc2.clear();    //     remove(c)                 remove(lc2.indexOf((Cour) c))
        		            lc = lc2;
        		            session.setAttribute("lc", lc); // Update the session attribute
        		            
        		            Iterator<Cour> i = lc.iterator();
        		            while(i.hasNext())
        		            {
        		            %>
        		            
        		                <h3>Nom = <%=i.next().getNom() %></h3>
        		            <%
        		            }
        		            prix = 0;
        		        }
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto p-4 p-lg-0">
                <a href="index.jso" class="nav-item nav-link active">Home</a>
                <a href="about.jsp" class="nav-item nav-link">About</a>
                <a href="courses.jsp" class="nav-item nav-link">Courses</a>
                <div class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown">Emplois du Temps</a>
        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-6 text-center mb-5">
                    ...
                    <h2 class="heading-section">Bac <%=matier %></h2>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <h4 class="text-center mb-4">Choisir votre paquit</h4>
                    <div class="table-wrap">
                        <table class="table">
                            ...
                            <tbody>
                               <%
                                   DAOPaquet daop = new DAOPaquet();
                                   Paquite p = daop.get_Paquit3(matier);
                                   if(p != null)
                                   {
									   if(p.getLc() != null)
									   {
                               ...
                               %>
                                <tr>
									...
									if (session.getAttribute("lc") != null) {
									%>
									    <h1>lc is not null</h1>
									    <%
									    lc3 = (List<Cour>) session.getAttribute("lc");
									    Cour cc = daoc.Find_ID(c.getId());
									    %>
									    <h4>cc = <%=cc.getNom() %></h4>
									    <h5>lc3 = <%=lc3 %></h5>
									    <h6>classe de lc3 = <%=lc3.getClass() %></h6>
									    <%
									    Boolean etat = false;
									    for(Cour cour : lc3)
									    {
									    	if(cour.getNom().equals(cc.getNom()))
									    	{
									    		etat = true;
									    	}
									    }
									    ...
									    %>
									        <td>
									            <a href="?m=<%=matier %>&op=supprimer&id=<%=c.getId() %>" class="btn btn-primary">Supprimer</a>
									        </td>
									    <%
									    }
									    else{
									    	...
									    	%>
									    	<h1>lc is null</h1>
									    	<td><a href="?m=<%=matier %>&op=ajouter&id=<%=c.getId() %>" class="btn btn-primary">Ajouter</a></td>
									    	<%
									    }
									} else 
									    {
                                ...
                                <%
									   ...
									   {
                                %>
                                	    <h1> le problem dans p != null && p.getc().equill </h1>
                                <%
									   }
                                %>
                                ...
                                <tfoot class="thead-primary">
                                    <tr>
                                        <th scope="row" class="scope border-bottom-0">Total</th>
                                        ...
                                        <%
                                        	if(session.getAttribute("e") != null)
                                        	{
                                        		Etudiant e = (Etudiant) session.getAttribute("e");
                                            %>
                                            	<a href="../etudiants?op=valider" class="btn btn-primary">Valider</a>
                                            <% 
                                        	}
                                        %>
                                        ...
                                        <th class="border-bottom-0" class="btn btn-primary"><%=prix %> dh</td>
                                    </tr>
                                </tfoot>
